function init() {
	defop.Google = {
		...
		fetchAttribution: async ({ layer }) => await fetchGoogleAttribution(layer),
		maxZoom: 22,
	...
	defop.Esri = {
		...
		maxNativeZoom: 19,
		maxZoom: 22,
		getattr: layer => fetch(`//static.arcgis.com/attribution/${layer}`).then(r => r.json()).then(r => r.contributors),
	...
	maps.Esri = L.layerGroup([
		...
		...defop.Esri,
		fetchAttribution: async (_) => (await Promise.all(Object.values(maps.Esri._layers).map(l => l.options.fetchAttribution(l.options)))).flat()
	});
	defop.Bing = {
		defaultAttribution: ["Microsoft"],
		fetchAttribution: async ({ layer }) => await fetchBingAttribution(layer),
		maxZoom: 22,
		q: ({ x, y, z }) => Array.from(Array(z).keys(), i => !!(x & 1 << i) + 2 * !!(y & 1 << i)).reverse().join(''),
	...
	defop.Here = {
		...
		maxNativeZoom: 20,
		maxZoom: 22,
		getattr: (layer, scheme) => fetch(`//1.${layer}.maps.ls.hereapi.com/maptile/2.1/copyright/newest?apikey=aRrXMN6rNeDunujbIgCqESvkttKlk4Pp2j5N7xTp4Ek`).then(r => r.json()),
...
async function fetchBingAttribution(e) {
	let query = ["South", "West", "North", "East"].map(d => map.getBounds()['get' + d]());
	query.unshift(e, map.getZoom());
	let url = `//dev.virtualearth.net/REST/V1/Imagery/Copyright/auto/${query.join('/')}?key=AuhiCJHlGzhg93IqUH_oCpl_-ZUrIE6SPftlyGYUvr9Amx5nzA-WqGcPquyFZl4L`;
	arr = await fetch(url).then((r) => r.json());
	arr = arr?.resourceSets?.map(s => s?.resources?.map(r => r?.imageryProviders))?.flat()?.flat() || [];
...
map.attributionControl._update = function() {
	let layerPromises = [],
		affectedLayers = [];
	for (let layer of [...Object.values(maps), ...Object.values(imagery)]) {
	...
	Promise.all(layerPromises).then((results) => {
		for (let [index, attribution] of results.entries()) {
